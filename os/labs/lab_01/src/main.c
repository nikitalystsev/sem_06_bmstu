...
myftw(char *pathname, Myfunc *func)
{
    ...
    printf("\nВремя работы функции path_alloc (без chdir): %.3f нс\n", cpu_time_used);
...
myftw_with_chdir(char *pathname, Myfunc *func)
{
    ...
    cpu_time_used = (double)(end - beg);
    printf("\nВремя работы функции path_alloc (chdir): %.3f нс\n", cpu_time_used);
...
void cmp_time(char *pathname)
{
    ...
    printf("\nВремя обхода дерева каталогов без chdir: %.3f нс\n", cpu_time_used);
    ...
    cpu_time_used = ((double)(end - beg)) / nreps;
    printf("Время обхода дерева каталогов с chdir: %.3f нс\n", cpu_time_used);
}